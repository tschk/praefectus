...
`ActionRequest.interaction_mode` is required and authority-bound. `interactive` permits the executor's advertised route. `background_only` refuses any action whose runtime capability cannot satisfy the host-selected session isolation. A model cannot assert that a session is isolated: `SessionIsolation` belongs to the executor configuration constructed by the host.
...
- A click with `VerificationPolicy::None` can succeed only as `executed_unverified`; it is never described as verified.
