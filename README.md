...
macOS delivers input aimed at a fenced element to that element's process through `CGEventPostToPid`, reported as the `per_process_event` delivery route. That delivery does not move the pointer or change the foreground application, so it is background-capable under the same sentinel guard as the target-addressed actions; it is not a noninterference claim, because the target process still receives synthetic input. A coordinate target carries no process identity, so `click`, `move`, and `drag` keep the global `pointer` route and are never background-capable. On every platform, an input action that cannot be delivered to a specific process is refused before any effect unless the host sets `PRAEFECTUS_ALLOW_GLOBAL_INPUT=1`. Linux and Windows have no per-process delivery path, so all of their input synthesis requires that opt-in. Praefectus reports these refusals as no effect, never as an unknown outcome, because nothing was dispatched.
