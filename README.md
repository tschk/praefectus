...
cargo run -- allow-global-input deny
```
...
macOS delivers input aimed at a fenced element to that element's process through `CGEventPostToPid`, reported as the `per_process_event` delivery route. That delivery does not move the pointer or change the foreground application, so it is background-capable under the same sentinel guard as the target-addressed actions; it is not a noninterference claim, because the target process still receives synthetic input. A coordinate target carries no process identity, so `click`, `move`, and `drag` keep the global `pointer` route and are never background-capable. On every platform, an input action that cannot be delivered to a specific process is refused before any effect unless the host opts in. The host can persist that opt-in with `praefectus allow-global-input allow`, which writes a private regular file under the Praefectus state directory, or set `PRAEFECTUS_ALLOW_GLOBAL_INPUT=1` for the current process. `PRAEFECTUS_ALLOW_GLOBAL_INPUT=0` denies global input even when the file exists. `praefectus allow-global-input deny` removes the persisted file. Linux and Windows have no per-process delivery path, so all of their input synthesis requires that opt-in. Praefectus reports these refusals as no effect, never as an unknown outcome, because nothing was dispatched.
