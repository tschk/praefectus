...
macOS delivers input aimed at a fenced element to that element's process through `CGEventPostToPid`, reported as the `per_process_event` delivery route. That delivery does not move the pointer or change the foreground application, so it is background-capable under the same sentinel guard as the target-addressed actions; it is not a noninterference claim, because the target process still receives synthetic input. A coordinate target carries no process identity, so `click`, `move`, and `drag` keep the global `pointer` route and are never background-capable. Praefectus refuses to post to the global event tap at all unless the host sets `PRAEFECTUS_ALLOW_GLOBAL_INPUT=1`; without it those actions fail before any effect rather than taking over the pointer.
