...
cargo run -- --no-yolo capabilities
cargo run -- allow-global-input
cargo run -- allow-global-input --no-yolo
cargo run -- allow-global-input allow
```
...
`NativeExecutor` is Praefectus-owned. It uses macOS Accessibility (AX), Windows UI Automation (UIA), and Linux AT-SPI2 to produce bounded semantic trees and execute actions on the active backend. All platforms expose fenced semantic `invoke` and `set_value`; `invoke` means the platform accessibility activation action and is never a pointer click. macOS additionally exposes fenced semantic `select_text`, which sets the accessibility selected-text range and succeeds only when the readback matches the requested range, and `perform_secondary_action`, which dispatches only an allowlisted accessibility action name that the live element still advertises. `perform_secondary_action` never dispatches the activation action that `invoke` owns, and the activation-equivalent names in the allowlist satisfy the same invokable fence as `invoke`. All platforms additionally expose `click`, `type_text`, `press`, `paste`, `hotkey`, `move`, and `scroll` as input actions, and `drag`, which requires a fenced coordinate target and carries a second fenced coordinate destination inside the action so both endpoints are bound by the action hash. Linux exposes effects only when the authenticated local X11 socket peer is a generation-bound native Xorg process, the server supplies RandR topology, and the XWAYLAND extension is absent; Wayland sessions use the xdg-desktop-portal RemoteDesktop interface for input and the Screenshot interface for capture. Windows additionally exposes one-step semantic scrolling when UIA reports the matching scroll pattern. macOS scrolls through the accessibility page-scroll action when the live element advertises it, which synthesizes no input at all; it falls back to a per-process wheel event otherwise, and reports the weaker per-process route in both cases rather than claiming the stronger one. Every semantic target binds its observation generation and expiry, backend provenance, process identity and generation, window identity, display geometry, opaque element identity, and element fingerprint. Missing, hidden, disabled, unstable, stale, ambiguous, or mismatched elements are rejected. Private state is restricted with Unix permissions or a protected current-user Windows ACL.
...
A host can supply an `OutcomeLedger` alongside its `Executor` and `AuthorityVerifier`. `OutcomeLedger::begin` is keyed by `OutcomeKey`, which is the operation ID and the canonical action hash, and returns `Dispatch`, `Recorded` with the stored terminal, or `Interrupted`. Praefectus consults it before dispatch: a recorded terminal is returned without re-dispatching the effect, and an operation the ledger already granted but never recorded terminates as `outcome_unknown` without dispatch. Praefectus records each terminal through the same ledger and never owns its storage. `NullOutcomeLedger` records nothing and always grants dispatch, which leaves the built-in per-operation file ledger as the only durability. `MemoryOutcomeLedger` is a process-local reference implementation for tests and single-process hosts. An attempt counter is deliberately not part of the key: a retried attempt of the same operation and action must resolve to the recorded outcome rather than dispatch again.

`INTERRUPTED_OUTCOME_MESSAGE` is the stable statement a host surfaces to the model when an outcome was lost: the outcome was not recorded, and the actual state must be verified before anything with side effects is redone. It appears in the `outcome_unknown` terminal message for a durable claim without a terminal receipt and for a ledger grant without a recorded outcome. It carries no screenshot bytes, typed text, clipboard contents, selectors, credentials, or backend detail.
...
On a shared native desktop, only target-addressed accessibility actions are background-capable. Praefectus samples the foreground process/window, keyboard or accessibility focus, and pointer before dispatch and again afterward. A change or an unavailable sentinel makes the result `outcome_unknown`, even when target verification succeeds. `unchanged_at_boundaries` states only what those two samples observed; it does not claim continuous noninterference. Receipts report the requested interaction mode, actual session isolation, delivery route, and context-preservation result. Global pointer delivery is never advertised as shared-desktop background execution.

macOS delivers input aimed at a fenced element to that element's process through `CGEventPostToPid`, reported as the `per_process_event` delivery route. That delivery does not move the pointer or change the foreground application, so it is background-capable under the same sentinel guard as the target-addressed actions; it is not a noninterference claim, because the target process still receives synthetic input. A coordinate target carries no process identity, so `click`, `move`, and `drag` keep the global `pointer` route and are never background-capable. On every platform, an input action that cannot be delivered to a specific process is allowed by default. Pass `--no-yolo` or run `praefectus allow-global-input --no-yolo` to persist the old refusal (a private regular file under the Praefectus state directory). `PRAEFECTUS_NO_YOLO=1` or `PRAEFECTUS_ALLOW_GLOBAL_INPUT=0` restricts the current process even without that file; `PRAEFECTUS_ALLOW_GLOBAL_INPUT=1` or `PRAEFECTUS_NO_YOLO=0` restores the unrestricted default. Linux and Windows have no per-process delivery path, so their input synthesis uses this global route unless `--no-yolo` is set. Praefectus reports these refusals as no effect, never as an unknown outcome, because nothing was dispatched.

macOS accessibility trees are read through `AXChildren`, falling back to `AXRows` and then `AXVisibleChildren` when the preceding axis is absent, empty, or larger than the traversal bound. The same precedence resolves observation paths, so an element's index means the same thing at observation and at dispatch. Some applications, notably Chromium and Electron hosts, only publish a full tree once `AXEnhancedUserInterface` or `AXManualAccessibility` is set on their application element. Praefectus never sets those implicitly: the host opts in with `PRAEFECTUS_AX_OPT_IN=AXEnhancedUserInterface,AXManualAccessibility`, and every attribute actually applied is recorded in `SemanticProvenance::host_opt_ins`, which is covered by the provenance hash that fences every target. A target observed under an opt-in therefore cannot be replayed against an observation taken without it.
...
- A durable claim without a terminal result becomes `outcome_unknown` on recovery, and its message carries the interrupted-outcome statement.
- A host outcome ledger resolves a re-presented operation from its recorded terminal instead of dispatching again.
...
Both features are off by default and macOS-only. `screencapturekit` enables the ScreenCaptureKit capture backend, which supplies the screen content hash that fences coordinate targets and reports `coordinate_capture`; it compiles a Swift bridge, so it requires an Xcode toolchain and links the Swift runtime. Without it `coordinate_capture` is false and coordinate targets do not resolve. `skylight` tries the undocumented `SLEventPostToPid` before the public per-process delivery path and falls back to it when the symbol is absent; it changes no reported route, capability, or guarantee.
